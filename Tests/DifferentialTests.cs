...
namespace FunctionFullAnalysis.Tests
{
    ...
    class DifferentialTests
    {
        ...
        void AssertDerivativeEqualToNumericDerivative(Expression<Func<double, double>> function)
        {
            ...
            double eps = 1e-7;
            var dfExpression = differentiator.Run(function);
        ...
        {
            var ex = Assert.Throws<NotImplementedException>(() => differentiator.Run(z => z.ToString().Length));
        ...
        {
            var ex = Assert.Throws<NotImplementedException>(() => differentiator.Run(z => Math.Max(z, 2 * z)));
